...
mlir::IntegerSet dropDimFromIntegerSet(mlir::IntegerSet set,
  ...
  //                          d_drop_dim → constant 0. Any constraint that was
  //                          purely a bound on d_drop_dim collapses to a
  //                          trivially-true constant and is eliminated by the
  ...
  mlir::IntegerSet substituted =
      set.replaceDimsAndSymbols(dim_replacements, /*symReplacements=*/{},
                                new_num_dims, set.getNumSymbols());
  return mlir::affine::simplifyIntegerSet(substituted);
}

mlir::IntegerSet pinDimInIntegerSet(mlir::IntegerSet set, unsigned pin_dim) {
  mlir::affine::FlatAffineValueConstraints cst(set);
  cst.addBound(mlir::presburger::BoundType::EQ, pin_dim, /*value=*/0);
  cst.removeRedundantConstraints();
  return cst.getAsIntegerSet(set.getContext());
}
...
mlir::AffineMap dropDimFromAffineMap(mlir::AffineMap map, unsigned drop_dim) {
  ...
  return map.dropResult(static_cast<int64_t>(drop_dim))
      .replaceDimsAndSymbols(dim_replacements, /*symReplacements=*/{},
...
std::optional<int64_t> getTripCount(mlir::IntegerSet set, unsigned dim_idx) {
  // getConstantBoundOnDimSize64 returns the smallest known constant bound on
  // (exclusive upper bound - lower bound) for the dimension, i.e. exactly its
  // trip count when the set constrains it to a rectangular range.
  mlir::affine::FlatAffineValueConstraints constraints(set);
  return constraints.getConstantBoundOnDimSize64(dim_idx);
}
...
/// must follow. MemRefs are excluded: base memrefs (desc_1, desc_2, addr_buf)
/// are out-of-scope, and the IAB memref is handled by the caller's pin-not-drop
/// / access-tile steps before propagateNarrowing is called.
...
mlir::LogicalResult propagateNarrowing(
  ...
  // erased_ops: old op pointers freed by a narrowOp rebuild (RAUW + erase).
  // A worklist entry queued before the rebuild may still carry a mlir::Value
  // whose defining op is one of these — the pointer value is safe to compare
  // against this set, but must never be dereferenced.
  llvm::DenseSet<mlir::Operation*> erased_ops;
  ...
  // AT, fill load; or access tile pin-not-drop) so the walk does not
  ...
  while (!worklist.empty()) {
    ...
    // If val's defining op was already erased-and-rebuilt by an earlier
    // worklist entry, val is now a stale reference to freed memory: RAUW
    // already rewired every use to the replacement value, and the
    // replacement's edges were already pushed when it was rebuilt. Drop this
    // entry before touching val any further (e.g. val.getUses() or
    // val.getDefiningOp()->getBlock() would read freed memory). DenseSet
    // membership only compares the pointer value, so this check itself never
    // dereferences the possibly-freed operation.
    if (mlir::Operation* val_def = val.getDefiningOp())
      if (erased_ops.count(val_def)) continue;
    ...
    if (dir == PropDir::DOWN) {
      ...
      for (mlir::OpOperand* use_ptr : uses) {
        ...
        // If narrowOp rebuilds `user` (RAUW + erase), `user` becomes a
        // dangling pointer. Capture the replacement via rebuilt_out instead
        // of probing `user` afterward (e.g. via getBlock()), which would be
        // a use-after-free.
        mlir::Operation* rebuilt_user = nullptr;
        if (!narrowed_ops.count(user)) {
          if (mlir::failed(narrowOp(user, tile_dim_to_drop, window_ivs,
                                    iab_rank, loc, ctx, &rebuilt_user)))
            return mlir::failure();
          narrowed_ops.insert(user);
          if (rebuilt_user) erased_ops.insert(user);
        }

        // Opaque ops (new_op + pre_narrowed) are already complete; do not
        // propagate through their operands or results. Safe even if `user`
        // was erased: opaque_ops never contains a rebuilt op's stale
        // predecessor, and this only compares the pointer value.
        if (opaque_ops.count(user)) continue;

        if (rebuilt_user) {
          // `user` is dead; propagate through the rebuilt op instead. Mark
          // it narrowed so subsequent visits of its results do not call
          // narrowOp on it again.
          narrowed_ops.insert(rebuilt_user);
          for (mlir::Value r : rebuilt_user->getResults())
            if (isNarrowable(r.getType()))
              worklist.push_back({r, PropDir::DOWN});
          for (mlir::Value o : rebuilt_user->getOperands())
            if (isNarrowable(o.getType())) worklist.push_back({o, PropDir::UP});
          continue;
        }
      ...
      // If narrowOp rebuilds `def_op` (RAUW + erase), `def_op` becomes a
      // dangling pointer. Track the replacement via `rebuilt` instead of
      // probing `def_op` afterward (e.g. via getBlock()), which would be a
      // use-after-free.
      ...
      if (!narrowed_ops.count(def_op)) {
        ...
        if (!mlir::isa<mlir::ktdp::LoadOp>(def_op)) {
          ...
          if (rebuilt) {
            // def_op was erased; record it so stale worklist entries that
            // still reference it are dropped instead of dereferenced.
            erased_ops.insert(def_op);
      ...
      // def_op was erased above; its results were already pushed DOWN.
      if (rebuilt) continue;
