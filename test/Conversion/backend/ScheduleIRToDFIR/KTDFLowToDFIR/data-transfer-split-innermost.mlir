...
// A transfer whose ONLY non-unit dim is the innermost one still splits: when
// the innermost size (128) is more than one vector (64 f16 lanes on
// sample_device), the innermost dim itself contributes the (sole) time
// dimension, with coefficient equal to the vector width (64) rather than 1.
// This shape was previously rejected outright (it has no outer dim to walk);
// it is now handled by the general innermost-split rule.

// CHECK: #[[$ATTR_0:.+]] = affine_map<(d0, d1, d2) -> (d0, d1, d2)>
// CHECK: #[[$ATTR_1:.+]] = affine_map<(d0) -> (0, 0, d0 * 64)>
// CHECK: #[[$ATTR_2:.+]] = affine_map<(d0) -> (d0)>
// CHECK: #[[$ATTR_3:.+]] = affine_set<(d0, d1, d2) : (d0 == 0, d1 == 0, d2 >= 0, -d2 + 63 >= 0)>
// CHECK: #[[$ATTR_4:.+]] = affine_set<(d0) : (d0 >= 0, -d0 + 1 >= 0)>

// CHECK-LABEL:   ktdf_arch.device @sample_device import("../../../../Dialect/KTDFArch/sample_device.mlir")

// CHECK-LABEL:   func.func @split_innermost_only() attributes {grid = [2]} {
...
// CHECK-NEXT:       %[[VAL_3:.*]] = memref.alloc() : memref<1x1x128xf16, "DDR">
// CHECK-NEXT:       %[[VAL_4:.*]] = memref.alloc() : memref<1x1x128xf16, "L1">
// CHECK-NEXT:       agen.composite_load_and_store src:%[[VAL_3]][0, 0, 0] dst:%[[VAL_4]][0, 0, 0]
...
// CHECK-NEXT:       } : memref<1x1x128xf16, "DDR">, memref<1x1x128xf16, "L1">
// CHECK-NEXT:     }
// CHECK-NEXT:     return
// CHECK-NEXT:   }
...
module {
  ...
  func.func @split_innermost_only() attributes {grid = [2]} {
