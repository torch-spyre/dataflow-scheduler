...
// Input is the output of IndirectAddrBufLegalization: a 1-D IAB (capacity
// already satisfied, no window loops) with a single entry loop already
...
// This pass must materialize a loop for it, leaving %arg8 retained (it
// passes both dense-packing, being innermost with stride 1, and full
// coverage, since its trip count equals %desc_1's dim-1 size of 64).
//
// The output descriptor access tile (%desc_2_at) already has its leading
// dimension pinned by IndirectAddrBufLegalization (to %i2, the entry loop
// IV) — this pass must continue pinning at the next dimension, not restart
// from 0.

// CHECK-LABEL: func.func @pin_continuation
...
// CHECK-SAME:          {{\[}}[[I3]], [[ARG8]]{{\]}}
// dropped: %i3's absorbed dim (dim 0, stride 4096) vanishes; only %arg8's
// dim (dim 1, full 64-element coverage) survives.
...
module @local_schedule_1 {
  ...
  func.func @pin_continuation() attributes {grid = [1 : index]} {
    ...
    scf.for %i2 = %c0_i2 to %c32 step %c1_i2 {
      ...
      // IndirectAddrBufLegalization already pinned this access tile's
      // leading dim to %i2 (rather than dropping it), so a
      // tensor.expand_shape bridges the compute
