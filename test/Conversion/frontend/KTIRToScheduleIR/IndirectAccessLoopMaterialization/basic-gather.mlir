...
// basic-gather.mlir test case: the IAB is 2-D (2x32), so that pass
...
// hoisted, loop-invariant IAB memref view (no iter-arg). Two remaining
// direct-subscript intermediate variables index %desc_1 directly: %arg3
// (dim 1, stride 4096) and %arg4 (dim 2, stride 1, full 64-element
// coverage).
//
...
// body — scf.if included — into %i3 for it to be perfectly nested.
...
// pinned: window+entry dims stay pinned as carried over; %i3's dim is newly
// pinned to a single point, and the direct dim keeps its full 64-element range.
  ...
  func.func @local_schedule_1() attributes {grid = [1 : index]} {
    ...
    scf.for %i1 = %c0_i1 to %c2 step %c1_i1 {
      ...
      scf.for %i2 = %c0_i2 to %c32 step %c1_i2 {
        ...
        %expanded = tensor.expand_shape %computed [[0, 1, 2], [3]] output_shape [1, 1, 2, 64]
            : tensor<2x64xf16> into tensor<1x1x2x64xf16>
        ...
        ktdp.store %expanded, %desc_2_at : tensor<1x1x2x64xf16>, !ktdp.access_tile<1x1x2x64xindex>
      }
