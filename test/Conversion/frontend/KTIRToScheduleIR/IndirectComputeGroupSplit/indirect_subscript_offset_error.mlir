// Verify that a non-zero offset on an indirect subscript is rejected rather
...
// to put the `+ 1`.  Deriving position(%arg5) would make the gather read the
// address resolved for element [arg5, arg6], i.e. one row too early, so the
// pass must diagnose the input instead.
//
