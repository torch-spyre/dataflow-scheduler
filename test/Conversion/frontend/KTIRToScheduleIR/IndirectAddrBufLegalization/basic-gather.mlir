...
//
// The window loop materializes the outer loop (%i1), narrowing the IAB
...
// by scf.if (%i2 == 0) and the ind_addr_buf memref threaded as an iter-arg.

// CHECK-LABEL: func.func @local_schedule_1
// CHECK:         scf.for [[I1:%arg[0-9]+]] = {{.*}} to %c2 step
// Sentinel outside the inner loop, iter-arg threading.
...
module @local_schedule_1 {
  ...
  func.func @local_schedule_1() attributes {grid = [1 : index]} {
    ...
    // in one shot.  IndirectAddrBufLegalization will split this into
    // per-row (window) and per-entry loops.
