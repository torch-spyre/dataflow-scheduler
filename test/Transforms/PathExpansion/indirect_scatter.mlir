...
// CHECK-SAME:          : memref<2x64xf16, {{.*}}, "DDR">, memref<2x64xf16, "L1">
...
// CHECK-NEXT:          : none, memref<2x64xf16, "L1">, memref<32xindex, {{.*}}, "IAB">, memref<64x2x64xf16, {{.*}}, "DDR">
// CHECK:           } {applicable_units = ["MNISU"]}

#map = affine_map<(d0, d1) -> (d0, d1)>
#set1 = affine_set<(d0, d1, d2) : (d0 >= 0, -d0 + 63 >= 0, d1 >= 0, -d1 + 1 >= 0, d2 >= 0, -d2 + 63 >= 0)>
#set2 = affine_set<(d1) : (d1 >= 0, -d1 + 31 >= 0)>
      ...
      scf.for %i2 = %c0 to %c32 step %c1 {
        ...
        ktdf.pipeline {
          ...
          ktdf.stage depends_in(%prv#2) depends_out(%prv#3) {
            ...
            ktdf.write_to_fifo %result, %prv#1
                : tensor<2x64xf16>, !ktdf.fifo.slot<"SFU" -> "DDR", 128xf16>
          } {applicable_units = ["SFU"]}

          // Stage 3 (store): scatter via IAB — dir_src is FIFO, will become L1 buffer
