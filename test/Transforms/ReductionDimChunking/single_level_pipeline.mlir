...
module {
  module {
    func.func @sum_nonstick_2core() attributes {grid = [2]} {
  ...
  module @local_schedule_0 {
    func.func @local_schedule_0() attributes {grid = [2]} {
      ...
      ktdf.pipeline {
        %5:8 = ktdf.private -> (memref<1x256x64xf16, #ktdp.memory_space<ct_local>>, !ktdf.fifo.slot<"LXLU" -> "SFP", 16384xf16>, !ktdf.fifo.slot<"SFP" -> "LXSU", 64xf16>, memref<1x64xf16, #ktdp.memory_space<ct_local>>, !ktdf.token, !ktdf.token, !ktdf.token, !ktdf.token) {
          %alloc = memref.alloc() : memref<1x256x64xf16, #ktdp.memory_space<ct_local>>
          %6 = ktdf.fifo.allocate() -> !ktdf.fifo.slot<"LXLU" -> "SFP", 16384xf16>
          %7 = ktdf.fifo.allocate() -> !ktdf.fifo.slot<"SFP" -> "LXSU", 64xf16>
          ...
          ktdf.private_yield %alloc, %6, %7, %alloc_2, %8, %9, %10, %11 : memref<1x256x64xf16, #ktdp.memory_space<ct_local>>, !ktdf.fifo.slot<"LXLU" -> "SFP", 16384xf16>, !ktdf.fifo.slot<"SFP" -> "LXSU", 64xf16>, memref<1x64xf16, #ktdp.memory_space<ct_local>>, !ktdf.token, !ktdf.token, !ktdf.token, !ktdf.token
        }
        ...
        ktdf.stage depends_in(%5#4) depends_out(%5#5) {
          ktdf.data_transfer from %5#0[0, 0, 0] size [1, 256, 64] to %5#1 size [16384] : memref<1x256x64xf16, #ktdp.memory_space<ct_local>>, !ktdf.fifo.slot<"LXLU" -> "SFP", 16384xf16>
        } {applicable_units = ["LXLU"]}
        ktdf.stage depends_in(%5#5) depends_out(%5#6) {
          %6 = ktdf.read_from_fifo %5#1 : <"LXLU" -> "SFP", 16384xf16> -> tensor<1x256x64xf16>
          ...
          ktdf.write_to_fifo %8, %5#2 : tensor<1x64xf16>, <"SFP" -> "LXSU", 64xf16>
        } {applicable_units = ["SFP"]}
        ktdf.stage depends_in(%5#6) depends_out(%5#7) {
          ktdf.data_transfer from %5#2 size [64] to %5#3[0, 0] size [1, 64] : !ktdf.fifo.slot<"SFP" -> "LXSU", 64xf16>, memref<1x64xf16, #ktdp.memory_space<ct_local>>
