...
# Creates a TableGen target and adds it to dataflow-scheduler-headers.
function(add_dataflow_scheduler_tablegen_target name)
  add_public_tablegen_target(${name})
  set(TABLEGEN_OUTPUT "" PARENT_SCOPE)
  add_dependencies(dataflow-scheduler-headers ${name})
endfunction()

# Creates a canonical MLIR dialect TableGen target and adds it to dataflow-scheduler-headers.
function(add_dataflow_scheduler_dialect dialect dialect_namespace)
  ...
  add_dataflow_scheduler_tablegen_target(DataflowScheduler${dialect}IncGen)
endfunction()

# Creates an MLIR TableGen documentation target and adds it to dataflow-scheduler-doc.
function(add_dataflow_scheduler_doc doc_filename output_file output_directory command)
  set(LLVM_TARGET_DEFINITIONS ${doc_filename}.td)
  set(TABLEGEN_OUTPUT "")
  tablegen(MLIR ${output_file}.md ${command} -allow-hugo-specific-features ${ARGN})
  set(_output "${DataflowScheduler_BINARY_DIR}/doc/${output_directory}${output_file}.md")
  add_custom_command(
    OUTPUT "${_output}"
  ...
  add_custom_target("DataflowScheduler${output_file}DocGen" DEPENDS "${_output}")
  add_dependencies(dataflow-scheduler-doc "DataflowScheduler${output_file}DocGen")
endfunction()
...
macro(add_dataflow_scheduler_tool name)
  cmake_parse_arguments(ARG "" "EXPORT_NAME" "" ${ARGN})

  if(NOT DataflowScheduler_BUILD_TOOLS)
  ...
  if(DataflowScheduler_BUILD_TOOLS AND ARG_EXPORT_NAME)
...
function(add_dataflow_scheduler_library name)
  ...
  if(ARG_GROUP)
    ...
    set_property(GLOBAL APPEND PROPERTY "DataflowScheduler_${ARG_GROUP}_LIBS" ${_qualified_name})
  endif()
  ...
  if(ARG_EXPORT_NAME)
    _install_dataflow_scheduler_library(${ARGV})
  endif()
endfunction()

# Creates a CAPI library target.
function(add_dataflow_scheduler_public_c_api_library name)
  cmake_parse_arguments(ARG "" "EXPORT_NAME" "" ${ARGN})
  ...
  add_dependencies(dataflow-scheduler-capi ${name})
endfunction()

function(_install_dataflow_scheduler_library name)
  cmake_parse_arguments(ARG "" "EXPORT_NAME" "" ${ARGN})
